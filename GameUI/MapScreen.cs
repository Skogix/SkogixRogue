...
namespace GameUI
{
	...
	{
		public GameUIMap Map { get; }

		public ScrollingConsole MapRenderer { get; }

		//Generate a map and display it.  Could just as easily pass it into
		public MapScreen(int mapWidth, int mapHeight, int viewportWidth
			, int viewportHeight)
		{
			...
			MapRenderer = Map.CreateRenderer(
				new XnaRect(0, 0, viewportWidth, viewportHeight)
			...
			Map.CalculateFOV(Map.ControlledGameObject.Position
				, Map.ControlledGameObject.FOVRadius, Radius.SQUARE);
			MapRenderer.CenterViewPortOnPoint(Map.ControlledGameObject.Position);
		}

		private void ControlledGameObjectChanged(object s
			, ControlledGameObjectChangedArgs e)
		{
			if (e.OldObject != null)
		...
		private static GameUIMap GenerateDungeon(int width, int height)
		{
			...
			var map = new GameUIMap(width, height);
			...
			QuickGenerators.GenerateDungeonMazeMap(tempMap, minRooms: 10, maxRooms: 20
				, roomMinSize: 5, roomMaxSize: 11);
			...
			for (int i = 0; i < 10; i++)
			{
				posToSpawn =
					map.WalkabilityView
						.RandomPosition(true); // Get a location that is walkable
				var goblin = new BasicEntity(Color.Red, Color.Transparent, 'g'
					, posToSpawn, (int) MapLayer.MONSTERS, isWalkable: false
					, isTransparent: true);
		...
		private static IGameObject SpawnTerrain(Coord position, bool mapGenValue)
		{
			...
			if (mapGenValue) // Floor
				return new BasicTerrain(Color.White, Color.Black, '.', position
