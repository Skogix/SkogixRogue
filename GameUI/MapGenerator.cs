	...
	public class MapGenerator
	{
			...
			int maxRoomSize)
		{
			...
			for (int i = 0; i < maxRooms; i++)
			{
				...
				int newRoomWidth = random.Next(minRoomSize, maxRoomSize);
				int newRoomHeight = random.Next(minRoomSize, maxRoomSize);
				// sätt pos som random inom mapsize
				int newRoomX = random.Next(0, mapWidth - newRoomWidth - 1);
				int newRoomY = random.Next(0, mapHeight - newRoomHeight - 1);
		...
		private void CreateRoom(Rectangle room)
		{
			...
			for (int x = room.MinExtentX + 1; x < room.MaxExtentX - 1; x++)
			{
				for (int y = room.MinExtentY + 1; y < room.MaxExtentY - 1; y++)
				{
			...
			// sätt väggar omkring
			List<Point> perimiter = GetBorderCellLocations(room);
			foreach (Point location in perimiter)
			{
				CreateWall(location);
			}
		}
		...
		{
			// sätter sidorna
			int left = room.MinExtentX; // left
			int right = room.MaxExtentX; // right
			int top = room.MinExtentY; // top
			int bottom = room.MaxExtentY; // bottom
			
			// gör en lista med "raka linjer" från rectangle
			List<Point> borderCells = new List<Point>();
			// top
			...
			// bottom
			borderCells.AddRange(GetTileLocationsAlongLine(left,bottom,right,bottom));
			// right
			borderCells.AddRange(GetTileLocationsAlongLine(right,top,right,bottom));
		...
		private IEnumerable<Point> GetTileLocationsAlongLine(int xStart, int yStart, int xEnd, int yEnd)
		{
			...
			int absXLineValue = Math.Abs(xEnd - xStart);
			int absYLineValue = Math.Abs(yEnd - yStart);

			// om start < end, lägg på ett
			// om start > end, dra av ett
			int xChangeValue = xStart < xEnd ? 1 : -1;
			int yChangeValue = yStart < yEnd ? 1 : -1;
			int diff = absXLineValue - absYLineValue;
